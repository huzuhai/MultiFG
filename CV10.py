...
#合并数据集
    ...
    print(f"drug_trairn的形状:{drug_train.shape}")
    print(f"adr_train的形状:{adr_train.shape}")
...
if __name__ == "__main__":
    ...
    for idx, (train_index, vali_index) in enumerate(kf.split(data_binary2)):
        # if idx == 3:
        #     break
        train_data = data_binary2.iloc[train_index, :]
        ...
        # #保存
        # with open('./result/loader/train_loader{folder}.pkl','wb') as f:
        #     dill.dump(train_loader, f)
        with open(f'./result/loader/vail_loader{folder}.pkl','wb') as f:
            dill.dump(vail_loader, f)
        # #加载
        # with open('./result/train_loader{folder}.pkl','rb') as f:
        #     train_loader = dill.load(f)
        # with open('./result/vail_loader{folder}.pkl','rb') as f:
