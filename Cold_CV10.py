...
def pre_data(train_data, vali_data, batch, folder):
    ...
    print(f"训练集药物与验证集药物的重叠数量：{len(set(data_vali_binary.drugname).intersection(set(data_train_binary.drugname)))}")
...
if __name__ == "__main__":
    ...
    for idx, (train_index, vali_index) in enumerate(kf.split(drug_data)):
        ...
        with open(f'./result/loader/vail_loader_cold{folder}.pkl','wb') as f:
        ...
        torch.save(model_auc.state_dict(), f'./result/model/dicit_model_cold{folder}.pth')
        # torch.save(model_auc, f'./result/model/whole_model_only{folder}.pth')

    pd.DataFrame(metrics_res).to_csv("./result/cold_start.csv")
    del data_binary2
    gc.collect()